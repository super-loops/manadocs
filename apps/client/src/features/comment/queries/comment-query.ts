...
export function useResolveCommentMutation() {
  ...
  return useMutation<IComment, Error, IResolveComment>({
    mutationFn: async (data) => {
      // Implement resolve comment API call here
      const response = await fetch(`/api/comments/${data.commentId}/resolve`, {
        method: 'PATCH',
    ...
    onSuccess: (updatedComment) => {
      ...
      notifications.show({ message: t("Comment status updated") });
    },
    onError: () => {
      notifications.show({
