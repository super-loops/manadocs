...
import { FileTask, InsertablePage } from '@manadocs/db/types/entity.types';
...
export class FileImportTaskService {
  ...
  async processGenericImport(opts: {
    ...
    const exportMetadata = await readExportMetadata(extractDir);
    ...
    for (const absPath of allFiles) {
      ...
      const pageMetadata = exportMetadata?.pages[encodedPath];
    ...
    sortedFolders.forEach((folderPath) => {
      ...
      if (!pagesMap.has(mdPath) && !pagesMap.has(htmlPath)) {
        ...
        if (!matched) {
          ...
          const placeholderMetadata = exportMetadata?.pages[encodedMdPath];
          pagesMap.set(mdPath, {
            id: v7(),
            slugId: generateSlugId(),
    ...
    const sortSiblings = (siblings: ImportPageNode[]) => {
      if (exportMetadata) {
        siblings.sort((a, b) => {
          const posA =
            exportMetadata.pages[encodedPathsMap.get(a.filePath)]?.position;
          const posB =
            exportMetadata.pages[encodedPathsMap.get(b.filePath)]?.position;
          if (posA && posB) {
