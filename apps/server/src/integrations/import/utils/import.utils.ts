...
export async function readExportMetadata(
  extractDir: string,
): Promise<ExportMetadata | null> {
  // Try manadocs-metadata.json first, fall back to docmost-metadata.json
  // for backwards compatibility with older exports.
  for (const filename of ['manadocs-metadata.json', 'docmost-metadata.json']) {
    try {
      ...
      if (
        (metadata.source === 'manadocs' || metadata.source === 'docmost') &&
        metadata.pages
      ) {
        return metadata;
      }
